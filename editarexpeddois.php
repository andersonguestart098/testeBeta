...
if (empty($dados['id'])) {
    ...
    $retorna = ['status' => false, 'msg' => "<div class='alert alert-danger' role='alert'>Erro: Necessário enviar o statusdep!</div>"];
}else {
    $query_usuario = "UPDATE exped2 SET datahora=:datahora, numeronf=:numeronf, exped=:exped, quemrecebeu=:quemrecebeu, statusdep=:statusdep WHERE id=:id"; //nunca bagunçar essa porra de função se não ele não altera no banco de dados
    ...
    $edit_usuario->bindParam(':statusdep', $dados['statusdep']);
