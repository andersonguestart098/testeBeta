...
if (!empty($pagina)) {
    ...
    $query_usuarios = "SELECT id, codentrega, conferente, numeronf, motorista, placa, cidadedestino, hodometrosaida, datahorasaida, obs FROM  saida ORDER BY id DESC LIMIT $inicio, $qnt_result_pg";
    $result_usuarios = $conn->prepare($query_usuarios);
    $result_usuarios->execute();
    //print_r($result_usuarios);

    // IMPORTANTE !! CUIDAAR NA TABELA SE FECHA </th> se fechar <th> ele fica fora de ordem.
    ...
    if (($result_usuarios) and ($result_usuarios->rowCount() != 0)) {
        ...
        $dados = "<div class=table-responsive-lg>
            <table class='table table-striped table table-hover table-bordered'>
                <thead class='table'>
                        ...
                        <th>HODOMETRO|SAIDA</th>
                        <th>DATAHORA|SAIDA</th>
                        <th>OBS</th>
                        <th>Ações</th>
                    </tr>
                </thead>
        ...
        while ($row_usuario = $result_usuarios->fetch(PDO::FETCH_ASSOC)) {
            ...
            $dados .= "<tr class=''>
                    ...
                    <td id='valor_codentrega$id'>$codentrega</td>
                    <td id='valor_conferente$id'>$conferente</td>
                    <td id='valor_numeronf$id'>$numeronf</td>
                    <td id='valor_motorista$id'>$motorista</td>
                    <td id='valor_placa$id'>$placa</td>
                    <td id='valor_cidadedestino$id'>$cidadedestino</td>
                    <td id='valor_hodometrosaida$id'>$hodometrosaida</td>
                    <td id='valor_datahorasaida$id'>$datahorasaida</td>
                    <td id='valor_obs$id'>$obs</td>
                    <td class='d-flex'>                        
        ...
        $query_pg = "SELECT COUNT(id) AS num_result FROM  saida";
