...
if (!empty($pagina)) {
    ...
    $query_usuarios = "SELECT id, datahora, cidade, numeronf, quemrecebeu, codentrega, motorista, statuslog FROM  logistica ORDER BY id DESC LIMIT $inicio, $qnt_result_pg";
    ...
    if (($result_usuarios) and ($result_usuarios->rowCount() != 0)) {
        ...
        while ($row_usuario = $result_usuarios->fetch(PDO::FETCH_ASSOC)) {
            extract($row_usuario);
            if($statuslog === 'ENTREGUE')
            $cordaLinha = 'bg-success';
            else if($statuslog === 'AGUARDANDO ROTA')
            $cordaLinha = 'bg-warning';
            else if($statuslog === 'MATERIAL RETORNOU')
            $cordaLinha = 'bg-info';
            else
            $cordaLinha = '';
            $dados .= "<tr class='$cordaLinha'>
                    ...
                    <td id='valor_cidade$id'>$cidade</td>
                    ...
                    <td id='valor_statuslog$id'>$statuslog</td>
                    <td class='d-flex'>                        
        ...
        $query_pg = "SELECT COUNT(id) AS num_result FROM  logistica";
