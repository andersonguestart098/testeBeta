...
function editar_registro(id) {
    ...
    exped.innerHTML = "<select name='quem' id='quem" + id + "'><option value='Exped.1'>Exped.1</option> </select>";
    quemrecebeu.innerHTML = "<input type='text' id='quemrecebeu_text" + id + "' value='" + quemrecebeu.innerHTML + "'>";
    statusdep.innerHTML = "<select name='status' id='status" + id + "'><option value='CLIENTE RETIROU'>CLIENTE RETIROU</option> <option value='AGUARDANDO CLIENTE'>AGUARDANDO CLIENTE</option> <option value='EM SEPARAÇÃO'>EM SEPARAÇÃO</option> </select>";
...
async function salvar_registro(id) {
    ...
    var exped_valor = document.getElementById("quem" + id).value;
    ...
    if (!resposta['status']) {
        ...
        document.getElementById("valor_exped" + id).innerHTML = exped_valor; // se faltar alguma linha de dado na hora de slavar ele continua com o campo editável !!
