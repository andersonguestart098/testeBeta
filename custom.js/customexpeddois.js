...
const listarUsuarios = async (pagina) => {
    ...
    const dados = await fetch("./listexpeddois.php?pagina=" + pagina);
...
cadForm.addEventListener("submit", async (e) => {
    ...
    const dados = await fetch("cadastrarexpeddois.php", {
        method: "POST",
        body: dadosForm,
    });
...
function editar_registro(id) {
    ...
    exped.innerHTML = "<input type='hidden' id='exped_text" + id + "' value='" + exped.innerHTML + "'>"; //prestar bem atenção se está referenciado o '_text' se não retorna valor nulo 'null'
    quemrecebeu.innerHTML = "<input type='hidden' id='quemrecebeu_text" + id + "' value='" + quemrecebeu.innerHTML + "'>";
    statusdep.innerHTML = "<select name='status' id='status" + id + "'><option value='PENDENTE'>PENDENTE</option><option value='AGUARDANDO CLIENTE'>AGUARDANDO CLIENTE</option><option value='CLIENTE RETIROU'>CLIENTE RETIROU</option> </select> ";
...
async function salvar_registro(id) {
    ...
    var quemrecebeu_valor = document.getElementById("quemrecebeu_text" + id).value;
    var statusdep_valor = document.getElementById("status" + id).value;
    ...
    const dados = await fetch("editarexpeddois.php", {
        method: "POST",
